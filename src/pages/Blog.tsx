  ...
  const { data: blogPosts = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('blog_posts')
        .select('id, title, excerpt, image_url, category, author, published_at, view_count, created_at, is_featured')
      ...
      <div className="min-h-screen">
        ...
        <main>
          ...
          <section className="py-12 sm:py-16 md:py-24 bg-background">
            <div className="container mx-auto px-4">
              <div className="text-center mb-8 sm:mb-10 max-w-7xl mx-auto">
                <h2 className="text-2xl sm:text-3xl font-bold mb-2">
                  {selectedCategory === "All" ? "All Articles" : selectedCategory}
                </h2>
                <p className="text-sm sm:text-base text-muted-foreground">
                  {filteredPosts.length} {filteredPosts.length === 1 ? 'article' : 'articles'} {searchQuery ? `matching "${searchQuery}"` : 'available'}
                </p>
              </div>
              {loading ? (
